...
const excelPath = path.join(__dirname, '../../filetuvung/Từ vựng HSK 2 3.0 new.xlsx');
...
rows.slice(1).forEach((r, idx) => {
  ...
  let formattedLessonTitle = lastLessonTitle ? `Bài ${lastLessonId}: ${lastLessonTitle}` : `Bài ${lastLessonId}`;
  ...
  parsedWords.push({
    ...
    lessonDesc: `Toàn bộ từ vựng ${formattedLessonTitle} chuẩn HSK 2 (v3.0)`,
    category,
    example_zh,
    example_vi,
    question,
...
// Separate HSK 2 (v3.0) items and non-HSK2 items
const existingHsk2Map = new Map();
db.forEach(w => {
  if (w.level === 2 && (w.hskVersion === '3.0' || !w.hskVersion) && !w.isCustom) {
    existingHsk2Map.set(`${w.word}_${w.lessonId || 1}`, w);
  }
});
...
const finalHsk2Items = parsedWords.map((pw, index) => {
  ...
  const existing = existingHsk2Map.get(key) || Array.from(existingHsk2Map.values()).find(e => e.word === pw.word);
...
console.log(`Updated ${updatedCount} existing HSK 2 items, added ${newCount} new items.`);

// Re-assemble database: non-HSK2 3.0 items + updated HSK 2 3.0 items
const nonHsk2Items = db.filter(w => !(w.level === 2 && (w.hskVersion === '3.0' || !w.hskVersion) && !w.isCustom));
