import"./modulepreload-polyfill-B5Qt9EMX.js";import"./particles-Dl8CQUZ9.js";let f={radicals:[],comparisons:[]};async function u(){try{let e=await fetch("/radicals_data.json");e.ok||(e=await fetch("/src/radicals_data.json")),e.ok&&(f=await e.json(),typeof m=="function"&&m())}catch(e){console.warn("Could not load radicals_data.json:",e)}}u();let p="50 bộ (1)",g=null,d=null,s=0,r=[];function w(e){if(!e)return;const n=e.trim();if(!n)return;if(d){try{d.pause(),d.currentTime=0,d.src=""}catch{}d=null}const i=parseFloat(localStorage.getItem("speech_playback_rate"))||1,a=`${window.location.origin.includes("5173")?"http://localhost:5000":window.location.origin}/api/tts?text=${encodeURIComponent(n)}&voice=baidu-female`,o=new Audio(a);o.playbackRate=i,d=o,o.play().catch(l=>{console.warn("Retrying Baidu female voice audio playback...",l),setTimeout(()=>{o.play().catch(y=>{if(console.error("Audio playback error:",y),"speechSynthesis"in window){window.speechSynthesis.cancel();const c=new SpeechSynthesisUtterance(n);c.lang="zh-CN",c.rate=i,window.speechSynthesis.speak(c)}})},200)})}window.speakText=w;window.switchRadicalPageTab=function(e){p=e,Object.entries({"50 bộ (1)":"tab-50-1","50 bộ (2)":"tab-50-2","50 bộ (3)":"tab-50-3","Còn lại":"tab-rest","So sánh":"tab-comp"}).forEach(([i,t])=>{const a=document.getElementById(t);a&&(i===e?a.classList.add("active"):a.classList.remove("active"))}),m()};function m(){const e=document.getElementById("radicals-content-area");if(e)if(p==="So sánh"){const n=f.comparisons||[];r=n.map((t,a)=>({id:`comp_${a}`,radical:`${t.rad1} / ${t.rad2}`,variant:"",pinyin:`${t.meaning1} vs ${t.meaning2}`,name:"Phân biệt",meaning:t.difference,note:t.difference,example:t.example,category:"So sánh"}));let i=`
    ...
    `;n.forEach((t,a)=>{i+=`
      ...
      `}),i+="</div>",e.innerHTML=i}else{const n=(f.radicals||[]).filter(t=>t.category===p);r=n;let i='<div class="grid-container">';n.forEach((t,a)=>{i+=`
        <div class="rad-card" onclick="window.openRadicalDetailByIndex(${a})">
  ...
  `,setTimeout(()=>{const i=document.getElementById("hero-tianzige-box");if(i&&window.HanziWriter){i.innerHTML="";const t=(e.radical||"").split("/")[0].trim();g=window.HanziWriter.create("hero-tianzige-box",t,{width:170,height:170,padding:10,showOutline:!0,strokeColor:"#dc2626",outlineColor:"#cbd5e1",showCharacter:!0}),g.animateCharacter()}},50))}function k(){const e=document.getElementById("mini-cards-grid");if(!e)return;let n="";r.forEach((i,t)=>{n+=`
    ...
    `;let a="";n.forEach((o,l)=>{const y=o.radical+(o.variant?` / ${o.variant}`:""),c=o.variant||o.radical;let h="",x="";for(let b=0;b<8;b++)h+=t(c,!0),x+=t("",!1);a+=`
      <div style="display: flex; align-items: stretch; border: 1px solid #94a3b8; border-radius: 6px; margin-bottom: 6px; page-break-inside: avoid; background: #fff;">
        <div style="width: 140px; padding: 4px 8px; border-right: 1.5px solid #64748b; display: flex; flex-direction: column; justify-content: center; background: #f8fafc;">
          <div style="font-size: 0.75rem; font-weight: 800; color: #2563eb;">STT ${l+1}</div>
    ...
    <div class="no-print" style="width: 100%; max-width: 860px; display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; gap: 12px; flex-wrap: wrap;">
      <div style="color: #ffffff; font-weight: 800; font-size: 1.1rem; display: flex; align-items: center; gap: 8px;">
        <i class="fa-solid fa-file-pdf" style="color: #10b981;"></i> Xem Trước Phiếu Tập Tô A4 — ${e}
      </div>
    ...
    <div id="printable-radical-worksheet-content" style="background: #ffffff !important; color: #0f172a !important; max-width: 860px; width: 100%; border-radius: 14px; padding: 24px; box-shadow: 0 25px 60px rgba(0,0,0,0.6); max-height: 82vh; overflow-y: auto; box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Be Vietnam Pro', sans-serif;">
      <div style="text-align: center; border-bottom: 2px solid #0f172a; padding-bottom: 10px; margin-bottom: 14px;">
        <h1 style="font-size: 1.25rem; font-weight: 800; margin: 0; text-transform: uppercase; color: #0f172a; letter-spacing: 0.3px; line-height: 1.3;">PHIẾU TẬP TÔ BỘ THỦ TIẾNG TRUNG — ${e.toUpperCase()}</h1>
  ...
  `,i.style.display="flex"};window.printWorksheetDocument=function(){window.print()};
