import"./modulepreload-polyfill-B5Qt9EMX.js";import"./particles-Dl8CQUZ9.js";let g={radicals:[],comparisons:[]};async function w(){try{const e=await fetch("/src/radicals_data.json");e.ok&&(g=await e.json(),typeof f=="function"&&f())}catch(e){console.warn("Could not load radicals_data.json:",e)}}w();let d="50 bộ (1)",c=null,l=null,s=0,a=[];function m(e){if(!e)return;const i=e.trim();if(!i)return;if(l){try{l.pause(),l.currentTime=0,l.src=""}catch{}l=null}const n=parseFloat(localStorage.getItem("speech_playback_rate"))||1,o=`${window.location.origin.includes("5173")?"http://localhost:5000":window.location.origin}/api/tts?text=${encodeURIComponent(i)}&voice=baidu-female`,r=new Audio(o);r.playbackRate=n,l=r,r.play().catch(y=>{console.warn("Retrying Baidu female voice audio playback...",y),setTimeout(()=>{r.play().catch(h=>{if(console.error("Audio playback error:",h),"speechSynthesis"in window){window.speechSynthesis.cancel();const p=new SpeechSynthesisUtterance(i);p.lang="zh-CN",p.rate=n,window.speechSynthesis.speak(p)}})},200)})}window.speakText=m;window.switchRadicalPageTab=function(e){d=e,Object.entries({"50 bộ (1)":"tab-50-1","50 bộ (2)":"tab-50-2","50 bộ (3)":"tab-50-3","Còn lại":"tab-rest","So sánh":"tab-comp"}).forEach(([n,t])=>{const o=document.getElementById(t);o&&(n===e?o.classList.add("active"):o.classList.remove("active"))}),f()};function f(){const e=document.getElementById("radicals-content-area");if(e)if(d==="So sánh"){const i=g.comparisons||[];a=i.map((t,o)=>({id:`comp_${o}`,radical:`${t.rad1} / ${t.rad2}`,variant:"",pinyin:`${t.meaning1} vs ${t.meaning2}`,name:"Phân biệt",meaning:t.difference,note:t.difference,example:t.example,category:"So sánh"}));let n=`
    ...
    `;i.forEach((t,o)=>{n+=`
        <div class="rad-card" onclick="window.openRadicalDetailByIndex(${o})">
      ...
      `}),n+="</div>",e.innerHTML=n}else{const i=(g.radicals||[]).filter(t=>t.category===d);a=i;let n='<div class="grid-container">';i.forEach((t,o)=>{n+=`
        <div class="rad-card" onclick="window.openRadicalDetailByIndex(${o})">
  ...
  `,m(e.radical),setTimeout(()=>{const n=document.getElementById("hero-tianzige-box");if(n&&window.HanziWriter){n.innerHTML="";const t=(e.radical||"").split("/")[0].trim();c=window.HanziWriter.create("hero-tianzige-box",t,{width:170,height:170,padding:10,showOutline:!0,strokeColor:"#dc2626",outlineColor:"#cbd5e1",showCharacter:!0}),c.animateCharacter()}},50))}function b(){const e=document.getElementById("mini-cards-grid");if(!e)return;let i="";a.forEach((n,t)=>{i+=`
    ...
    `}),e.innerHTML=i,setTimeout(()=>{const n=document.getElementById(`mini-card-${s}`);n&&n.scrollIntoView({behavior:"smooth",block:"nearest",inline:"center"})},100)}window.animateRadicalStroke=function(){c&&c.animateCharacter()};document.addEventListener("keydown",e=>{const i=document.getElementById("radicals-flashcard-view");i&&i.style.display!=="none"&&(e.key==="ArrowLeft"?window.prevRadicalFlashcard():e.key==="ArrowRight"?window.nextRadicalFlashcard():(e.key===" "||e.key==="Spacebar")&&(e.preventDefault(),a[s]&&m(a[s].radical)))});document.addEventListener("DOMContentLoaded",()=>{f()});
