(function(){function y(){const e=window.location.pathname.toLowerCase(),i=window.location.search.toLowerCase();return e==="/"||e.endsWith("/index.html")?i.includes("tab=flashcards")?"flashcards":i.includes("view=roadmap")||window.location.hash.includes("roadmap")?"roadmap":"home":e.includes("video-dictation")?i.includes("mode=shadowing")?"shadowing":"dictation":e.includes("reading-practice")?"reading":e.includes("chinese-phonetics")?"phonetics":e.includes("chinese-radicals")?"radicals":e.includes("hanzi-writer")?"hanzi":e.includes("hsk-grammar")?"grammar":e.includes("lesson-texts")?"texts":e.includes("detail-list")?"vocabulary":e.includes("quiz-game")?"games":e.includes("han-viet-rules")?"rules":e.includes("rank")?"rank":""}function h(){try{const e=localStorage.getItem("user")||localStorage.getItem("hongtai_current_user")||localStorage.getItem("currentUser")||sessionStorage.getItem("user");if(e)return JSON.parse(e)}catch{}return null}function p(){const e=h();document.querySelectorAll(".app-sidebar, .global-app-sidebar").forEach(a=>{const n=a.querySelector(".user-name, #user-display-name"),s=a.querySelector(".user-sub, #user-display-email"),o=a.querySelector(".user-role-badge, #user-display-role"),r=a.querySelector(".sidebar-avatar-wrap"),l=a.querySelector(".sidebar-auth-action-item");if(e&&(e.name||e.email)){const f=e.name||e.displayName||(e.email?e.email.split("@")[0]:"Học viên"),b=e.email||"",m=e.role==="super_admin"?"Super Admin":e.role==="admin"?"Admin":e.role==="teacher"?"Giáo viên":"Học viên",u=e.picture||e.avatar||"";n&&(n.textContent=f),s&&(s.textContent=b),o&&(o.textContent=m),r&&(u?r.innerHTML=`<img class="user-avatar-img" src="${u}" alt="Avatar" style="display: block; width: 44px; height: 44px; border-radius: 50%; object-fit: cover;">`:r.innerHTML='<div class="user-avatar sidebar-avatar-placeholder"><i class="fa-solid fa-user"></i></div>'),l&&(l.innerHTML=`
          ...
          `)}else n&&(n.textContent="Khách (Chưa đăng nhập)"),s&&(s.textContent="Đăng nhập để lưu tiến độ học"),o&&(o.textContent="Khách"),r&&(r.innerHTML='<div class="user-avatar sidebar-avatar-placeholder"><i class="fa-solid fa-user"></i></div>'),l&&(l.innerHTML=`
          ...
          `)})}function v(){const e=y(),i=h(),a=i?i.name||i.displayName||(i.email?i.email.split("@")[0]:"Học viên"):"Khách (Chưa đăng nhập)",n=i?i.email||"":"Đăng nhập để lưu tiến độ học",s=i?i.role==="super_admin"?"Super Admin":i.role==="admin"?"Admin":i.role==="teacher"?"Giáo viên":"Học viên":"Khách",o=i&&(i.picture||i.avatar)?i.picture||i.avatar:"";return`
          ...
          <div class="user-profile sidebar-profile-card" onclick="window.toggleGlobalUserDropdown && window.toggleGlobalUserDropdown(event)"
            ...
            <div style="display: flex; align-items: center; gap: 10px; min-width: 0; flex: 1;">
              <div class="sidebar-avatar-wrap" style="flex-shrink: 0;">
                ${o?`<img class="user-avatar-img" src="${o}" alt="Avatar" style="display: block; width: 42px; height: 42px; border-radius: 50%; object-fit: cover; border: 2px solid var(--accent-blue, #38bdf8);">`:'<div class="user-avatar sidebar-avatar-placeholder" style="width: 42px; height: 42px; border-radius: 50%; background: linear-gradient(135deg, #3b82f6, #8b5cf6); display: flex; align-items: center; justify-content: center; color: white;"><i class="fa-solid fa-user"></i></div>'}
              </div>
              <div class="user-info" style="min-width: 0; flex: 1; display: flex; flex-direction: column; overflow: hidden;">
                <span class="user-name" style="font-weight: 700; font-size: 0.92rem; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${a}</span>
    ...
    `}window.openGlobalSidebar=function(){p();const e=window.location.pathname==="/"||window.location.pathname.endsWith("/index.html");if(e&&window.innerWidth>=768){document.body.classList.remove("sidebar-collapsed"),localStorage.setItem("sidebar_collapsed","false");return}const i=document.querySelector(".app-sidebar")||document.getElementById("global-app-sidebar"),a=document.querySelector(".sidebar-backdrop")||document.getElementById("global-sidebar-backdrop");i&&(i.classList.add("open","active"),i.style.pointerEvents="auto"),a&&(!e||window.innerWidth<768)&&a.classList.add("active"),document.body.classList.add("sidebar-open")},window.closeGlobalSidebar=function(){const e=document.querySelector(".app-sidebar")||document.getElementById("global-app-sidebar"),i=document.querySelector(".sidebar-backdrop")||document.getElementById("global-sidebar-backdrop");e&&e.classList.remove("open","active"),i&&i.classList.remove("active"),document.body.classList.remove("sidebar-open")},window.toggleGlobalSidebar=function(){if((window.location.pathname==="/"||window.location.pathname.endsWith("/index.html"))&&window.innerWidth>=768){window.toggleSidebarCollapse?window.toggleSidebarCollapse():document.body.classList.toggle("sidebar-collapsed");return}const i=document.querySelector(".app-sidebar")||document.getElementById("global-app-sidebar");i&&(i.classList.contains("open")||i.classList.contains("active")||document.body.classList.contains("sidebar-open"))?window.closeGlobalSidebar():window.openGlobalSidebar()},window.toggleGlobalUserDropdown=function(e){e&&(e.stopPropagation(),typeof e.preventDefault=="function"&&e.preventDefault());const i=e&&e.target?e.target.closest(".user-dropdown"):null;i?i.classList.toggle("show-menu"):document.querySelectorAll(".user-dropdown").forEach(n=>n.classList.toggle("show-menu"))},window.toggleUserDropdown=window.toggleGlobalUserDropdown,window.handleGlobalLogout=async function(e){e&&(e.stopPropagation(),typeof e.preventDefault=="function"&&e.preventDefault());try{await fetch("/api/auth/logout",{method:"POST",credentials:"include"})}catch{}if(localStorage.removeItem("user"),localStorage.removeItem("currentUser"),localStorage.removeItem("hongtai_user"),localStorage.removeItem("hongtai_current_user"),localStorage.removeItem("session_token"),sessionStorage.removeItem("user"),typeof google<"u"&&google.accounts&&google.accounts.id)try{google.accounts.id.disableAutoSelect()}catch{}typeof window.handleLogout=="function"?window.handleLogout(e):window.location.reload()},window.openLoginPrompt=function(){const e=document.getElementById("app-login-modal")||document.getElementById("auth-required-modal");e?(e.style.display="flex",document.body.style.overflow="hidden"):window.location.href="/?login=true"},document.addEventListener("keydown",function(e){e.key==="Escape"&&(window.closeGlobalSidebar(),document.querySelectorAll(".user-dropdown.show-menu").forEach(i=>i.classList.remove("show-menu")))}),document.addEventListener("click",function(e){e.target.closest(".user-dropdown")||document.querySelectorAll(".user-dropdown.show-menu").forEach(i=>i.classList.remove("show-menu"))});function w(){const e=window.location.pathname==="/"||window.location.pathname.endsWith("/index.html");e&&window.innerWidth>=768&&(document.body.classList.remove("sidebar-collapsed"),localStorage.setItem("sidebar_collapsed","false"));let i=document.querySelector(".sidebar-backdrop");if(i?e&&i.classList.add("on-index"):(i=document.createElement("div"),i.className="sidebar-backdrop"+(e?" on-index":""),i.id="global-sidebar-backdrop",document.body.appendChild(i)),i.addEventListener("click",function(o){window.closeGlobalSidebar()}),!document.querySelector(".app-sidebar")&&!e){const o=document.createElement("div");o.id="global-sidebar-mount",o.innerHTML=v(),document.body.insertBefore(o.firstElementChild,document.body.firstChild)}p(),window.addEventListener("storage",p),window.addEventListener("user-auth-changed",p),setTimeout(p,500),setTimeout(p,1500),document.querySelectorAll(".app-sidebar, .global-app-sidebar").forEach(o=>{o.addEventListener("click",r=>{r.target.closest(".user-dropdown")||document.querySelectorAll(".user-dropdown.show-menu").forEach(l=>l.classList.remove("show-menu")),r.stopPropagation()})});const n=()=>{document.querySelectorAll(".app-sidebar .sidebar-item, .global-app-sidebar .sidebar-item, .app-sidebar .sidebar-subitem, .global-app-sidebar .sidebar-subitem").forEach(o=>{o.classList.contains("sidebar-dropdown-toggle")||(o.style.pointerEvents="auto",o.addEventListener("click",()=>{window.innerWidth<=900&&setTimeout(()=>{window.closeGlobalSidebar()},120)}))})};n(),setTimeout(n,600),x();const s=()=>{document.querySelectorAll(".menu-toggle-btn, .global-hamburger-btn, .sidebar-open-btn, .top-menu-btn, #top-sidebar-toggle-btn, #sidebar-expand-float-btn, .sidebar-expand-float-btn, #mobile-nav-toggle-btn").forEach(o=>{o.onclick=window.toggleGlobalSidebar})};s(),setTimeout(s,500)}function x(){if(document.getElementById("menu-bubble-widget"))return;const e=window.location.pathname==="/"||window.location.pathname.endsWith("/index.html"),i=document.createElement("div");i.className="menu-bubble-widget"+(e?" on-index-page":""),i.id="menu-bubble-widget";const a=document.createElement("button");a.className="menu-bubble-btn",a.id="menu-bubble-btn",a.title="Mở Menu Danh Mục (☰)",a.setAttribute("aria-label","Mở Menu Danh Mục"),a.innerHTML='<i class="fa-solid fa-bars"></i>',i.appendChild(a),document.body.appendChild(i),k(i,a)}function k(e,i){let a=!1,n=0,s=0,o=0,r=0,l=!1;function f(t){return t.touches&&t.touches.length>0?{x:t.touches[0].clientX,y:t.touches[0].clientY}:{x:t.clientX,y:t.clientY}}function b(t){if(t.button&&t.button!==0)return;const d=f(t);n=d.x,s=d.y;const c=e.getBoundingClientRect();o=c.left,r=c.top,l=!1,a=!0,e.style.transition="none"}function m(t){if(!a)return;const d=f(t),c=d.x-n,g=d.y-s;if(!l&&(Math.abs(c)>6||Math.abs(g)>6)&&(l=!0),l){t.cancelable&&t.preventDefault();const L=window.innerWidth-e.offsetWidth-10,S=window.innerHeight-e.offsetHeight-10,C=Math.max(10,Math.min(L,o+c)),M=Math.max(10,Math.min(S,r+g));e.style.left=`${C}px`,e.style.top=`${M}px`,e.style.bottom="auto",e.style.right="auto"}}function u(t){if(a)if(a=!1,e.style.transition="all 0.3s cubic-bezier(0.16, 1, 0.3, 1)",!l)window.toggleGlobalSidebar();else{const d=e.getBoundingClientRect(),c=window.innerWidth/2;d.left+d.width/2<c?(e.style.left="16px",e.style.right="auto"):(e.style.left=`${window.innerWidth-d.width-16}px`,e.style.right="auto")}}i.addEventListener("touchstart",b,{passive:!1}),window.addEventListener("touchmove",m,{passive:!1}),window.addEventListener("touchend",u),i.addEventListener("mousedown",b),window.addEventListener("mousemove",m),window.addEventListener("mouseup",u)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",w):w()})();
