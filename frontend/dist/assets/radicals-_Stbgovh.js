import"./modulepreload-polyfill-B5Qt9EMX.js";import"./particles-Dl8CQUZ9.js";let I={radicals:[],comparisons:[]};async function q(){try{let e=await fetch("/radicals_data.json");e.ok||(e=await fetch("/src/radicals_data.json")),e.ok&&(I=await e.json(),typeof M=="function"&&M())}catch(e){console.warn("Could not load radicals_data.json:",e)}}q();let B="50 bộ (1)",W=null,C=null,b=0,c=[];function A(e){if(!e)return;const i=e.trim();if(!i)return;if(C){try{C.pause(),C.currentTime=0,C.src=""}catch{}C=null}const n=parseFloat(localStorage.getItem("speech_playback_rate"))||1,r=`${window.location.origin.includes("5173")?"http://localhost:5000":window.location.origin}/api/tts?text=${encodeURIComponent(i)}&voice=baidu-female`,p=new Audio(r);p.playbackRate=n,C=p,p.play().catch(d=>{console.warn("Retrying Baidu female voice audio playback...",d),setTimeout(()=>{p.play().catch(l=>{if(console.error("Audio playback error:",l),"speechSynthesis"in window){window.speechSynthesis.cancel();const f=new SpeechSynthesisUtterance(i);f.lang="zh-CN",f.rate=n,window.speechSynthesis.speak(f)}})},200)})}window.speakText=A;window.switchRadicalPageTab=function(e){B=e,Object.entries({"50 bộ (1)":"tab-50-1","50 bộ (2)":"tab-50-2","50 bộ (3)":"tab-50-3","Còn lại":"tab-rest","So sánh":"tab-comp"}).forEach(([n,t])=>{const r=document.getElementById(t);r&&(n===e?r.classList.add("active"):r.classList.remove("active"))}),M()};function M(){const e=document.getElementById("radicals-content-area");if(e)if(B==="So sánh"){const i=I.comparisons||[];c=i.map((t,r)=>({id:`comp_${r}`,radical:`${t.rad1} / ${t.rad2}`,variant:"",pinyin:`${t.meaning1} vs ${t.meaning2}`,name:"Phân biệt",meaning:t.difference,note:t.difference,example:t.example,category:"So sánh"}));let n=`
    ...
    `;i.forEach((t,r)=>{n+=`
      ...
      `}),n+="</div>",e.innerHTML=n}else{const i=(I.radicals||[]).filter(t=>t.category===B);c=i;let n='<div class="grid-container">';i.forEach((t,r)=>{n+=`
        <div class="rad-card" onclick="window.openRadicalDetailByIndex(${r})">
      ...
      <div class="no-print" style="padding:10px 20px; background:#1e293b; border-bottom:1px solid rgba(255,255,255,0.08); display:flex; flex-wrap:wrap; align-items:center; gap:16px; font-size:0.85rem; color:#cbd5e1;">
        ...
        <div style="display:flex; align-items:center; gap:6px;">
          ...
          <select id="radical-print-scope" onchange="updateRadicalWorksheetPreviewConfig()" style="background:#0f172a; border:1px solid #f59e0b; color:#fff; padding:4px 10px; border-radius:8px; font-weight:700; outline:none; cursor:pointer;">
            <option value="all" ${S==="all"?"selected":""}>Tất cả bộ thủ (${B})</option>
            <option value="single" ${S==="single"?"selected":""}>Chỉ bộ thủ đang chọn</option>
          </select>
        </div>
  ...
  `,i.style.display="flex",updateRadicalWorksheetPreviewConfig()};window.closeRadicalPrintWorksheetModal=function(){const e=document.getElementById("print-radical-worksheet-modal");e&&(e.style.display="none");const i=document.getElementById("seasonal-particle-canvas");i&&localStorage.getItem("particles_enabled")!=="false"&&(i.style.display="block")};window.updateRadicalWorksheetPreviewConfig=function(){const e=document.getElementById("radical-print-scope"),i=document.getElementById("radical-print-layout"),n=document.getElementById("radical-print-rows"),t=document.getElementById("radical-print-trace-rows"),r=document.getElementById("radical-print-show-pinyin"),p=document.getElementById("radical-print-show-meaning");e&&(S=e.value),i&&(_=i.value),n&&(F=n.value),t&&(O=t.value),r&&(U=r.checked),p&&(X=p.checked);let d=B;(!d||d==="So sánh"||d==="Còn lại")&&(d="50 bộ (1)");let l=(I.radicals||[]).filter(f=>f.category===d);if(l.length===0&&(l=(I.radicals||[]).slice(0,50)),S==="single"&&c[b]){const f=c[b];f&&f.radical?(l=l.filter(H=>H.radical===f.radical),l.length===0&&(l=[f])):l=l.slice(0,1)}Z(l)};function Z(e){const i=document.getElementById("radical-print-worksheet-content-wrap");if(!i)return;const n=new Date,t=`${n.getDate()}/${n.getMonth()+1}/${n.getFullYear()}`,r=13,p=48,d=parseInt(F)||4,l=O,f=U,H=X;function j(o,a=""){return`
      ...
      ">
        ...
        ${a}
      </div>`}const N=[];let V="";if(_==="fit_multi"){let o=e.length,a=3;d<=2?a=5:d===3?a=4:d===4?a=3:d>=5&&(a=2);let g=Math.ceil(o/a)||1;const y=document.getElementById("modal-radical-print-total-pages");y&&(y.textContent=`${o} bộ thủ (${g} trang)`);for(let h=0;h<g;h++){const u=e.slice(h*a,(h+1)*a);let $="";u.forEach((s,z)=>{const k=h*a+z,T=s.radical,m=s.radical+(s.variant?` / ${s.variant}`:""),P=[T];function L(x){let K="";for(let D=0;D<r;D++){let R=!1;(l==="all"||l==="1"&&x===0||l==="2"&&(x===0||x===1)||l==="3"&&(x===0||x===1||x===2))&&(R=!0);let G="";R&&(G=`<span style="
                font-size:${p*.72}px;
              ...
              ">${T}</span>`),K+=j(p,G)}return`<div style="display:flex;">${K}</div>`}let v="";for(let x=0;x<d;x++)v+=L(x);const E=`radical-stroke-steps-modal-${k}`;N.push({id:E,wordChars:P});const Y=`
          <div style="
          ...
          ">
            <div style="display:flex; justify-content:space-between; align-items:center; padding-bottom:6px; border-bottom:1px dashed #dcfce7;">
              <div style="display:flex; align-items:baseline; gap:12px; flex-wrap:wrap;">
                ...
                ${H?`<span style="font-size:0.9rem; color:#374151; font-weight:700;">${s.name?`${s.name} (${s.meaning||""})`:s.meaning||""}</span>`:""}
              </div>
      ...
      `}}else{const o=document.getElementById("modal-radical-print-total-pages");o&&(o.textContent=`${e.length} trang`),e.forEach((a,g)=>{const y=a.radical,h=a.radical+(a.variant?` / ${a.variant}`:""),u=[y];function $(m){let P="";for(let L=0;L<r;L++){let v=!1;(l==="all"||l==="1"&&m===0||l==="2"&&(m===0||m===1)||l==="3"&&(m===0||m===1||m===2))&&(v=!0);let E="";v&&(E=`<span style="
              font-size:${p*.72}px;
              font-family:'LXGW WenKai Lite','Kaiti','STKaiti','Kai','PingFang SC','Noto Serif SC',serif;
              font-weight:normal; color:#111827; opacity:0.18;
              line-height:1; position:relative; z-index:2; pointer-events:none;
            ">${y}</span>`),P+=j(p,E)}return`<div style="display:flex;">${P}</div>`}let w="";for(let m=0;m<d;m++)w+=$(m);const s=`radical-stroke-steps-modal-${g}`;N.push({id:s,wordChars:u});const z=`
        <div style="
          border:2px solid #22c55e; border-radius:8px;
        ...
        ">
          <div style="display:flex; justify-content:space-between; align-items:center; padding-bottom:8px; border-bottom:1px dashed #dcfce7;">
            <div style="display:flex; align-items:baseline; gap:16px; flex-wrap:wrap;">
              ...
              ${H?`<span style="font-size:0.95rem; color:#374151; font-weight:600;">${a.name?`${a.name} (${a.meaning||""})`:a.meaning||""}</span>`:""}
            </div>
