import"./modulepreload-polyfill-B5Qt9EMX.js";import"./particles-Dl8CQUZ9.js";let g={radicals:[],comparisons:[]};async function w(){try{let e=await fetch("/radicals_data.json");e.ok||(e=await fetch("/src/radicals_data.json")),e.ok&&(g=await e.json(),typeof f=="function"&&f())}catch(e){console.warn("Could not load radicals_data.json:",e)}}w();let d="50 bộ (1)",c=null,l=null,s=0,o=[];function h(e){if(!e)return;const i=e.trim();if(!i)return;if(l){try{l.pause(),l.currentTime=0,l.src=""}catch{}l=null}const n=parseFloat(localStorage.getItem("speech_playback_rate"))||1,a=`${window.location.origin.includes("5173")?"http://localhost:5000":window.location.origin}/api/tts?text=${encodeURIComponent(i)}&voice=baidu-female`,r=new Audio(a);r.playbackRate=n,l=r,r.play().catch(m=>{console.warn("Retrying Baidu female voice audio playback...",m),setTimeout(()=>{r.play().catch(y=>{if(console.error("Audio playback error:",y),"speechSynthesis"in window){window.speechSynthesis.cancel();const p=new SpeechSynthesisUtterance(i);p.lang="zh-CN",p.rate=n,window.speechSynthesis.speak(p)}})},200)})}window.speakText=h;window.switchRadicalPageTab=function(e){d=e,Object.entries({"50 bộ (1)":"tab-50-1","50 bộ (2)":"tab-50-2","50 bộ (3)":"tab-50-3","Còn lại":"tab-rest","So sánh":"tab-comp"}).forEach(([n,t])=>{const a=document.getElementById(t);a&&(n===e?a.classList.add("active"):a.classList.remove("active"))}),f()};function f(){const e=document.getElementById("radicals-content-area");if(e)if(d==="So sánh"){const i=g.comparisons||[];o=i.map((t,a)=>({id:`comp_${a}`,radical:`${t.rad1} / ${t.rad2}`,variant:"",pinyin:`${t.meaning1} vs ${t.meaning2}`,name:"Phân biệt",meaning:t.difference,note:t.difference,example:t.example,category:"So sánh"}));let n=`
    ...
    `;i.forEach((t,a)=>{n+=`
      ...
      `}),n+="</div>",e.innerHTML=n}else{const i=(g.radicals||[]).filter(t=>t.category===d);o=i;let n='<div class="grid-container">';i.forEach((t,a)=>{n+=`
        <div class="rad-card" onclick="window.openRadicalDetailByIndex(${a})">
  ...
  `,setTimeout(()=>{const n=document.getElementById("hero-tianzige-box");if(n&&window.HanziWriter){n.innerHTML="";const t=(e.radical||"").split("/")[0].trim();c=window.HanziWriter.create("hero-tianzige-box",t,{width:170,height:170,padding:10,showOutline:!0,strokeColor:"#dc2626",outlineColor:"#cbd5e1",showCharacter:!0}),c.animateCharacter()}},50))}function b(){const e=document.getElementById("mini-cards-grid");if(!e)return;let i="";o.forEach((n,t)=>{i+=`
    ...
    `}),e.innerHTML=i,setTimeout(()=>{const n=document.getElementById("mini-cards-grid"),t=document.getElementById(`mini-card-${s}`);if(n&&t){const a=t.offsetLeft-n.clientWidth/2+t.offsetWidth/2;n.scrollTo({left:Math.max(0,a),behavior:"smooth"})}},100)}window.animateRadicalStroke=function(){c&&c.animateCharacter()};document.addEventListener("keydown",e=>{const i=document.getElementById("radicals-flashcard-view");i&&i.style.display!=="none"&&(e.key==="ArrowLeft"?window.prevRadicalFlashcard():e.key==="ArrowRight"?window.nextRadicalFlashcard():(e.key===" "||e.key==="Spacebar")&&(e.preventDefault(),o[s]&&h(o[s].radical)))});document.addEventListener("DOMContentLoaded",()=>{f()});
