import"./global_sidebar-B_BNhh9i.js";/* empty css              */import"./particles-DBEuVAM5.js";window.pdfjsLib&&(window.pdfjsLib.GlobalWorkerOptions.workerSrc="/vendor/pdfjs/pdf.worker.min.js");(function(){const E="";let G=[],re="all",T="",y=null,f=null,c=1,B=0,ee=!1,te=null,N=1.25,$="fitWidth",de=null,I=localStorage.getItem("hongtai_reader_theme")||"dark",V=[],q="page",x=[],we="#fef08a",Ee=0,Te=0,K,O,D,ne,L,M,_,A,d,b,Be,k,ge,ue,he,U,R,F,X,oe,v,ie,se,me,C,ae,ce,fe;document.addEventListener("DOMContentLoaded",()=>{Le(),Ce(),Se(),He(),$e()});function Le(){K=document.getElementById("documents-grid"),O=document.getElementById("documents-search-input"),D=document.getElementById("documents-search-clear"),ne=document.querySelectorAll(".documents-filter-btn"),L=document.getElementById("documents-bookshelf-view"),M=document.getElementById("documents-catalog-view"),_=document.getElementById("btn-view-bookshelf"),A=document.getElementById("btn-view-catalog"),d=document.getElementById("ebook-reader-modal"),b=document.getElementById("pdf-render-canvas"),b&&(Be=b.getContext("2d")),k=document.getElementById("reader-page-input"),ge=document.getElementById("reader-total-pages"),ue=document.getElementById("reader-book-title"),he=document.getElementById("reader-book-meta"),U=document.getElementById("reader-btn-prev"),R=document.getElementById("reader-btn-next"),F=document.getElementById("reader-float-prev"),X=document.getElementById("reader-float-next"),oe=document.getElementById("reader-loading-overlay"),v=document.getElementById("reader-comments-drawer"),ie=document.getElementById("reader-comments-list"),se=document.getElementById("reader-comment-input"),me=document.getElementById("btn-submit-comment"),C=document.getElementById("reader-notes-drawer"),ae=document.getElementById("reader-notes-list"),ce=document.getElementById("reader-note-input"),fe=document.getElementById("btn-submit-note")}function Ce(){O&&O.addEventListener("input",s=>{T=s.target.value.trim().toLowerCase(),D&&(D.style.display=T?"block":"none"),W()}),D&&D.addEventListener("click",()=>{O&&(O.value=""),T="",D.style.display="none",W()}),ne.forEach(s=>{s.addEventListener("click",()=>{ne.forEach(r=>r.classList.remove("active")),s.classList.add("active"),re=s.getAttribute("data-category")||"all",W()})}),U&&U.addEventListener("click",Y),R&&R.addEventListener("click",Q),F&&F.addEventListener("click",Y),X&&X.addEventListener("click",Q),k&&k.addEventListener("change",s=>{const r=parseInt(s.target.value,10);!isNaN(r)&&r>=1&&r<=B?S(r):k.value=c});const t=document.getElementById("reader-zoom-in"),e=document.getElementById("reader-zoom-out"),n=document.getElementById("reader-zoom-fit");t&&t.addEventListener("click",()=>{$="custom",N=Math.min(3,N+.2),S(c)}),e&&e.addEventListener("click",()=>{$="custom",N=Math.max(.6,N-.2),S(c)}),n&&n.addEventListener("click",()=>{$=$==="fitWidth"?"fitPage":"fitWidth",u($==="fitWidth"?"Đã chỉnh: Vừa chiều rộng":"Đã chỉnh: Vừa trang"),S(c)});const o=document.getElementById("reader-theme-toggle");o&&o.addEventListener("click",qe);const a=document.getElementById("reader-btn-back");a&&a.addEventListener("click",ve);const i=document.getElementById("reader-viewport");if(i){let s=0,r=0;i.addEventListener("touchstart",m=>{m.touches&&m.touches.length===1&&(s=m.touches[0].clientX,r=m.touches[0].clientY)},{passive:!0}),i.addEventListener("touchend",m=>{if(m.changedTouches&&m.changedTouches.length===1){const ke=m.changedTouches[0].clientX-s,Ue=m.changedTouches[0].clientY-r;Math.abs(ke)>45&&Math.abs(ke)>Math.abs(Ue)*1.3&&(ke<0?Q():Y())}},{passive:!0})}let l=null;window.addEventListener("resize",()=>{clearTimeout(l),l=setTimeout(()=>{d&&d.style.display==="flex"&&f&&S(c)},250)});const p=document.getElementById("reader-toggle-comments");p&&p.addEventListener("click",De);const h=document.getElementById("drawer-close-comments");h&&h.addEventListener("click",()=>H(v));const w=document.getElementById("reader-toggle-notes");w&&w.addEventListener("click",ze);const P=document.getElementById("drawer-close-notes");P&&P.addEventListener("click",()=>H(C)),document.querySelectorAll(".drawer-filter-tab").forEach(s=>{s.addEventListener("click",()=>{document.querySelectorAll(".drawer-filter-tab").forEach(r=>r.classList.remove("active")),s.classList.add("active"),q=s.getAttribute("data-mode")||"page",J()})}),document.querySelectorAll(".color-dot").forEach(s=>{s.addEventListener("click",()=>{document.querySelectorAll(".color-dot").forEach(r=>r.classList.remove("active")),s.classList.add("active"),we=s.getAttribute("data-color")||"#fef08a"})}),me&&me.addEventListener("click",_e),fe&&fe.addEventListener("click",Ve),window.addEventListener("keydown",s=>{!d||d.style.display!=="flex"||["INPUT","TEXTAREA"].includes(document.activeElement.tagName)||(s.key==="ArrowRight"||s.key==="PageDown"||s.key===" "?(s.preventDefault(),Q()):s.key==="ArrowLeft"||s.key==="PageUp"?(s.preventDefault(),Y()):s.key==="Escape"&&ve())});const Z=document.getElementById("reader-viewport");Z&&(Z.addEventListener("touchstart",s=>{s.touches.length===1&&(Ee=s.touches[0].clientX,Te=s.touches[0].clientY)},{passive:!0}),Z.addEventListener("touchend",s=>{if(s.changedTouches.length===1){const r=s.changedTouches[0].clientX-Ee,m=s.changedTouches[0].clientY-Te;Math.abs(r)>60&&Math.abs(r)>Math.abs(m)*1.5&&(r<0?Q():Y())}},{passive:!0}))}function Se(){d&&d.addEventListener("contextmenu",t=>(t.preventDefault(),u("🔒 Tài liệu được bảo vệ bản quyền - Chỉ hỗ trợ đọc trực tuyến trên Tiếng Trung HongTai",!0),!1)),window.addEventListener("keydown",t=>{if(d&&d.style.display==="flex"&&(t.ctrlKey||t.metaKey)&&["s","p","u"].includes(t.key.toLowerCase()))return t.preventDefault(),u("🔒 Tính năng in và lưu tệp bị vô hiệu hóa để bảo vệ bản quyền tác giả.",!0),!1}),b&&b.addEventListener("dragstart",t=>t.preventDefault())}async function He(){try{const e=await(await fetch(`${E}/api/books`,{headers:j()})).json();e.success&&Array.isArray(e.books)&&(G=e.books,Pe(),W(),$e())}catch(t){console.error("Failed to load books catalog:",t),K&&(K.innerHTML=`
        ...
        `)}}window.switchDocsView=function(t){(!L||!M)&&(L=document.getElementById("documents-bookshelf-view"),M=document.getElementById("documents-catalog-view"),_=document.getElementById("btn-view-bookshelf"),A=document.getElementById("btn-view-catalog")),t==="bookshelf"?(L&&(L.style.display="block"),M&&(M.style.display="none"),_&&_.classList.add("active"),A&&A.classList.remove("active")):(L&&(L.style.display="none"),M&&(M.style.display="block"),_&&_.classList.remove("active"),A&&A.classList.add("active"))};function $e(){const t=L||document.getElementById("documents-bookshelf-view");if(!t)return;function e(o){if(!o)return null;const a=G.find(p=>p.id===o),i=ye(o),l=a&&a.userProgress||i;return l&&l.lastPage>1?l:null}const n=[{id:"shelf-hsk2",accentColor:"#0284c7",mascotImg:"/assets/logo.png",title:"Giáo Trình Chuẩn HSK 2.0",subtitle:"Bộ giáo trình 6 cấp độ (HSK 1 - HSK 6)",books:[{level:1,title:"Giáo Trình Chuẩn HSK 1",coverUrl:"/covers/hsk2/hsk1.jpg",bookId:null,desc:"Giáo trình chuẩn HSK 1 bao gồm 15 bài học nhập môn kèm file dịch tiếng Việt chuẩn của TS. Nguyễn Thị Minh Hồng."},{level:2,title:"Giáo Trình Chuẩn HSK 2",coverUrl:"/covers/hsk2/hsk2.jpg",bookId:null,desc:"Giáo trình chuẩn HSK 2 bao gồm 15 bài học sơ cấp nâng cao mở rộng vốn câu giao tiếp thực tế."},{level:3,title:"Giáo Trình Chuẩn HSK 3",coverUrl:"/covers/hsk2/hsk3.jpg",bookId:null,desc:"Giáo trình chuẩn HSK 3 bao gồm 20 bài học củng cố ngữ pháp trung cấp và diễn đạt mở rộng."},{level:4,title:"Giáo Trình Chuẩn HSK 4",coverUrl:"/covers/hsk2/hsk4.jpg",bookId:null,desc:"Giáo trình chuẩn HSK 4 gồm 2 tập Thượng và Hạ với 20 chủ đề chuyên sâu."},{level:5,title:"Giáo Trình Chuẩn HSK 5 (Tập Dưới)",coverUrl:"/covers/hsk2/hsk5.jpg",bookId:"book_8e6cd7",desc:"Giáo trình chuẩn HSK 5 Tập Dưới (HSK 5 chuẩn下.pdf) bản số hóa sắc nét, đọc trực tuyến mượt mà."},{level:6,title:"Giáo Trình Chuẩn HSK 6",coverUrl:"/covers/hsk2/hsk6.jpg",bookId:null,desc:"Giáo trình chuẩn HSK 6 cao cấp dành cho người học thành thạo ngôn ngữ."}]},{id:"shelf-hsk3",accentColor:"#10b981",mascotImg:"/assets/logo.png",title:"Giáo Trình HSK 3.0 Mới",subtitle:"Bộ giáo trình tiêu chuẩn quốc tế mới nhất (Cấp 1 - Cấp 4)",books:[{level:1,title:"新 HSK 教程 1 (HSK 3.0)",coverUrl:"/covers/hsk3/hsk1.jpg",bookId:null,desc:"Giáo trình HSK cấp độ 1 tiêu chuẩn 3.0 mới nhất do NXB Giáo dục & Nghiên cứu Giảng dạy Ngôn ngữ Bắc Kinh xuất bản."},{level:2,title:"新 HSK 教程 2 (HSK 3.0)",coverUrl:"/covers/hsk3/hsk2.jpg",bookId:"book_74e5v6",desc:"Toàn bộ giáo trình HSK 2 mới chuẩn quốc tế 3.0 với đầy đủ bài khóa và hội thoại."},{level:3,title:"新 HSK 教程 3 (HSK 3.0)",coverUrl:"/covers/hsk3/hsk3.jpg",bookId:"book_3b4cws",desc:"Giáo trình HSK 3 mới chuẩn 3.0 củng cố hệ thống ngữ pháp và 1000 từ vựng cốt lõi."},{level:4,title:"新 HSK 教程 4 (Tập Trên - HSK 3.0)",coverUrl:"/covers/hsk3/hsk4.jpg",bookId:"book_32gwjz",desc:"Giáo trình HSK 4 mới Tập Trên chuẩn v3.0 kèm 10 bài học trung cấp cơ bản."}]},{id:"shelf-hanngu",accentColor:"#f59e0b",mascotImg:"/assets/logo.png",title:"Giáo Trình Hán Ngữ 6 Cuốn",subtitle:"Bản dịch song ngữ Trung - Việt chuẩn Đại học (Tập 1 - 6)",books:[{volNum:1,title:"Giáo Trình Hán Ngữ 1",subtitle:"Bản dịch song ngữ",bookId:"book_m03mz1",color:"#eab308",tag:"Quyển 1 Thượng",desc:"Giáo trình Hán ngữ cơ sở quyển 1 chuẩn quốc tế, rèn luyện phát âm Pinyin, nét bút và giao tiếp nhập môn."},{volNum:2,title:"Giáo Trình Hán Ngữ 2",subtitle:"Bản dịch song ngữ",bookId:"book_82zwgt",color:"#16a34a",tag:"Quyển 1 Hạ",desc:"Giáo trình Hán ngữ bộ 6 tập quyển 2, bản dịch chú giải tiếng Việt chuẩn xác cho học viên mới bắt đầu."},{volNum:3,title:"Giáo Trình Hán Ngữ 3",subtitle:"Bản dịch song ngữ",bookId:"book_8emthg",color:"#ea580c",tag:"Quyển 2 Thượng",desc:"Giáo trình Hán ngữ quyển 3 (Tập 2 Thượng), bước ngoặt củng cố ngữ pháp trung cấp và diễn đạt mở rộng."},{volNum:4,title:"Giáo Trình Hán Ngữ 4",subtitle:"Bản dịch song ngữ",bookId:"book_ow9jfp",color:"#2563eb",tag:"Quyển 2 Hạ",desc:"Giáo trình Hán ngữ quyển 4 (Tập 2 Hạ), nâng cao vốn từ vựng chuyên đề, câu phức và đối thoại chuyên sâu."},{volNum:5,title:"Giáo Trình Hán Ngữ 5",subtitle:"Bản dịch song ngữ",bookId:"book_tn7sl6",color:"#9333ea",tag:"Quyển 3 Thượng",desc:"Giáo trình Hán ngữ quyển 5 (Tập 3 Thượng) nâng cao khả năng phân tích ngữ văn và nghị luận."},{volNum:6,title:"Giáo Trình Hán Ngữ 6",subtitle:"Bản dịch song ngữ",bookId:null,color:"#0f766e",tag:"Quyển 3 Hạ",desc:"Giáo trình Hán ngữ quyển 6 (Tập 3 Hạ) hoàn thiện kỹ năng đọc hiểu và văn phong bản xứ cao cấp."}]}];t.innerHTML=n.map(o=>{const a=o.books.map(i=>{const l=e(i.bookId),p=!!i.bookId;let h="";l?h=`<span class="comic-book-status-badge reading"><i class="fa-solid fa-bookmark"></i> P.${l.lastPage}</span>`:p?h='<span class="comic-book-status-badge available"><i class="fa-solid fa-book-open"></i> Đọc ngay</span>':h='<span class="comic-book-status-badge coming-soon"><i class="fa-solid fa-clock"></i> Sắp ra</span>';let w="";i.coverUrl?w=`<img src="${i.coverUrl}" alt="${g(i.title)}" class="comic-book-cover-img" loading="lazy">`:w=`
          ...
          `;const P=g(i.title),Z=i.coverUrl?g(i.coverUrl):"",s=g(i.desc||""),r=i.bookId?`'${i.bookId}'`:"null",m=i.level||i.volNum?`${i.level||i.volNum}`:"null";return`
      ...
      `}).join("")}window.openShelfBook=function(t,e,n,o,a){if(t){window.openBookReader(t);return}const i=document.getElementById("pending-book-modal"),l=document.getElementById("pending-modal-cover"),p=document.getElementById("pending-modal-title"),h=document.getElementById("pending-modal-desc"),w=document.getElementById("pending-modal-actions");if(i&&p&&h){l&&(l.src=n||"/assets/logo.png",l.alt=e),p.textContent=e,h.textContent=o||"Bản số hóa chất lượng cao đang được cập nhật để mang lại trải nghiệm đọc tốt nhất cho bạn.";let P=`
      ...
      `,a&&(a.innerHTML=`
        ...
        `),t.style.display="flex")};function Pe(){const t={all:G.length};G.forEach(e=>{t[e.category]=(t[e.category]||0)+1}),ne.forEach(e=>{const n=e.getAttribute("data-category"),o=e.querySelector(".documents-filter-count");o&&(o.textContent=t[n]||0)})}function W(){if(!K)return;let t=G.filter(e=>{const n=re==="all"||e.category===re,o=!T||e.titleVi&&e.titleVi.toLowerCase().includes(T)||e.titleOriginal&&e.titleOriginal.toLowerCase().includes(T)||e.category&&e.category.toLowerCase().includes(T)||e.description&&e.description.toLowerCase().includes(T);return n&&o});if(t.length===0){K.innerHTML=`
      ...
      `}).join("")}async function _e(){if(!se||!y)return;const t=se.value.trim();if(!t){u("Vui lòng nhập nội dung bình luận.",!0);return}try{const n=await(await fetch(`${E}/api/books/${y.id}/comments`,{method:"POST",headers:{"Content-Type":"application/json",...j()},body:JSON.stringify({page:c,content:t})})).json();n.success&&n.comment?(V.unshift(n.comment),se.value="",Ie(),J(),u("Đã gửi bình luận thành công!")):u(n.error||"Lỗi gửi bình luận",!0)}catch(e){console.error("Error posting comment:",e),u("Không thể gửi bình luận.",!0)}}async function Ae(t){try{const n=await(await fetch(`${E}/api/books/${t}/notes`,{headers:j()})).json();n.success&&Array.isArray(n.notes)&&(x=n.notes,be(),le())}catch(e){console.error("Failed to fetch notes:",e)}}function be(){const t=document.getElementById("reader-notes-count-badge");t&&(t.textContent=x.length,t.style.display=x.length>0?"inline-block":"none")}function ze(){if(!C)return;H(v),C.classList.contains("open")?H(C):(C.classList.add("open"),le())}function le(){if(ae){if(x.length===0){ae.innerHTML=`
        <div style="text-align: center; padding: 40px 10px; color: #94a3b8;">
