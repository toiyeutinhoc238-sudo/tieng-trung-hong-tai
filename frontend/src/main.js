...
function renderLearningTianzige(word) {
  ...
  // Determine cell size based on character count (expands paper for >=2 chars)
  let cellSize = 150;
  if (cleanChars.length === 2) cellSize = 125;
  else if (cleanChars.length >= 3) cellSize = 100;

  // Single Merged Outer Frame ("Tờ giấy dài gộp ô")
    ...
    flashcard.innerHTML = `
      ...
      <div style="display: flex; gap: 24px; align-items: flex-start; justify-content: space-between; flex-wrap: wrap; text-align: left; width: 100%; ${hasExercise ? 'margin-bottom: 24px;' : ''}">
        ...
        <div style="display: flex; flex-direction: column; align-items: center; gap: 10px; min-width: 150px;">
          <div id="roadmap-tianzige-container" style="display: flex; justify-content: center; align-items: center;"></div>
